        ...
        <>
          ...
          {!loading && allVariants.length > 0 && (
            <div className="flex gap-1.5 flex-wrap">
              <button onClick={() => setFilter('all')} className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] font-black transition-all ${filter === 'all' ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'}`}>
                Todos {allVariants.length}
              </button>
              {outCount > 0 && (
                <button onClick={() => setFilter('out')} className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] font-black transition-all ${filter === 'out' ? 'bg-red-600 text-white' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>
                  Sin stock {outCount}
                </button>
              )}
              {lowCount > 0 && (
                <button onClick={() => setFilter('low')} className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] font-black transition-all ${filter === 'low' ? 'bg-amber-500 text-white' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'}`}>
                  Stock bajo {lowCount}
                </button>
              )}
              {okCount > 0 && <button onClick={() => setFilter('ok')} className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] font-black transition-all ${filter === 'ok' ? 'bg-emerald-600 text-white' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'}`}>OK {okCount}</button>}
            </div>
          )}
