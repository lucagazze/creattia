          ...
          return (
            <div key={d} className="relative py-0.5 flex items-center justify-center w-full">
              ...
              {hasRange && (isInRange || (isHovering && d !== displayStart && d !== displayEnd)) && (
                <div className="absolute inset-y-0.5 left-0 right-0 bg-emerald-50 dark:bg-emerald-500/10" />
              )}
              {hasRange && isStart && (
                <div className="absolute inset-y-0.5 right-0 left-1/2 bg-emerald-50 dark:bg-emerald-500/10" />
              )}
              {hasRange && isEnd && (
                <div className="absolute inset-y-0.5 left-0 right-1/2 bg-emerald-50 dark:bg-emerald-500/10" />
              )}
              ...
              <button
                ...
                className={`h-7 w-7 sm:h-8 sm:w-8 text-[10px] sm:text-[11px] font-bold transition-all relative flex items-center justify-center rounded-full ${
                  isSelected || (since && !until && hovering && (d === since || d === hovering))
                    ? 'bg-emerald-600 text-white z-10 shadow-md shadow-emerald-200 dark:shadow-none'
                    : isFuture ? 'text-zinc-200 dark:text-zinc-800 cursor-default' :
                    'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'
                } ${isToday && !isSelected && !(since && !until && hovering && (d === since || d === hovering)) ? 'text-emerald-600 dark:text-emerald-500 ring-1 ring-emerald-100 dark:ring-emerald-900/30' : ''}`}
              >
                {d.split('-')[2]}
              </button>
            </div>
