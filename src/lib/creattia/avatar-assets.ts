...
export async function resolveAvatarReferences(input: {
	...
	if (input.mode === 'saved') {
		...
		for (const row of evenlySample(rows || [], 6)) {
	...
	const files = (input.directImages || []).filter((entry): entry is File => entry instanceof File && entry.size > 0).slice(0, 7);
	...
	if (files.length > 6) throw new Error('Podés usar hasta 6 fotos por avatar.');
	...
	const images: VisualImage[] = [];
	for (const file of evenlySample(files, 6)) {
