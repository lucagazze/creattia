...
export default function MetaAdsPage() {
  ...
  const [prevAdInsightsMap, setPrevAdInsightsMap] = useState<Record<string, any>>({});
  ...
  const analyzeCreativeUrl = async (imageUrl: string | null, isVideo: boolean, fatigueContext?: any): Promise<any> => {
    ...
    const res = await fetch('/api/scrape-all', {
      ...
      body: JSON.stringify({ type: 'analyze-creative', frames: frame ? [frame] : [], imageUrl, isVideo, fatigueContext }),
    });
  ...
  const handleTabChange = async (tab: 'comments' | 'metrics', imageUrl?: string, isVideo?: boolean, videoUrl?: string | null) => {
    if (tab === 'metrics') {
      ...
      // Si el anuncio muestra fatiga, la IA además propone ángulos nuevos para reemplazarlo
      const fatigue = selectedAd ? getAdFatigue(selectedAd.ad) : null;
      const fatigueContext = fatigue
        ? { adName: selectedAd?.name, body: selectedAd?.body, signals: fatigue.signals }
        : undefined;
      analyzeCreativeUrl(imageUrl || null, isVideo || false, fatigueContext)
        .then(result => {
  ...
  const fetchAds = (since?: string, until?: string) => {
    ...
    if (!accountId) {
      setActiveAds([]); setAdInsightsMap({}); setPrevAdInsightsMap({}); setCampaignMap({}); setAdsetMap({});
      setResolvedThumbnails({}); setResolvedDetails({}); setResolvingIds({});
      setLoading(false);
      return;
    }
    ...
    // Período anterior equivalente: base de comparación para detectar fatiga creativa
    const prevTr = getPrevPeriod(tr.since, tr.until);
    const adFields = 'ad_id,ad_name,campaign_id,campaign_name,adset_id,adset_name,spend,impressions,reach,frequency,inline_link_click_ctr,inline_link_clicks,actions,cost_per_action_type,action_values,purchase_roas,video_30_sec_watched_actions,video_p100_watched_actions';
    Promise.all([
      ...
      metaAds.getAdInsightsForAccount(accountId, adFields, prevTr).catch(() => []),
    ]).then(([adsRes, insightsRes, campsRes, adsetsRes, prevInsightsRes]) => {
      const prevByAdId: Record<string, any> = {};
      (prevInsightsRes || []).forEach((i: any) => { if (i.ad_id) prevByAdId[i.ad_id] = i; });
      setPrevAdInsightsMap(prevByAdId);
  ...
  // ── Fatiga creativa ──────────────────────────────────────────────────────────
  // Compara el período actual contra el anterior equivalente. Señales: CTR en caída,
  // frecuencia alta y CPA en alza. 2+ señales = fatigado, 1 = vigilar.
  const countAdResults = (ins: any): number => {
    const actions = ins?.actions || [];
    const get = (t: string) => { const a = actions.find((x: any) => x.action_type === t || x.action_type === `offsite_conversion.fb_pixel_${t}`); return a ? parseInt(a.value) : 0; };
    const msg = actions.find((x: any) => x.action_type === 'onsite_conversion.messaging_conversation_started_7d' || x.action_type === 'onsite_conversion.messaging_first_reply');
    return get('purchase') + get('lead') + (msg ? parseInt(msg.value) : 0);
  };

  const getAdFatigue = (ad: any): { level: 'high' | 'watch'; signals: string[] } | null => {
    const st = ad?.effective_status || ad?.status;
    if (st !== 'ACTIVE') return null;
    const curr = adInsightsMap[ad.id];
    const prev = prevAdInsightsMap[ad.id];
    if (!curr || !prev) return null;
    const spend = parseFloat(curr.spend || 0);
    const prevSpend = parseFloat(prev.spend || 0);
    // Sin gasto comparable en ambos períodos, la señal no es confiable
    if (spend < 3 || prevSpend < 3) return null;
    const signals: string[] = [];
    const ctr = parseFloat(curr.inline_link_click_ctr || 0);
    const prevCtr = parseFloat(prev.inline_link_click_ctr || 0);
    if (prevCtr > 0 && ctr < prevCtr * 0.8) signals.push(`CTR cayó ${Math.round((1 - ctr / prevCtr) * 100)}% (${prevCtr.toFixed(2)}% → ${ctr.toFixed(2)}%)`);
    const freq = parseFloat(curr.frequency || 0);
    if (freq >= 2.5) signals.push(`Frecuencia alta: ${freq.toFixed(1)} — la audiencia ya lo vio varias veces`);
    const results = countAdResults(curr);
    const prevResults = countAdResults(prev);
    const cpa = results > 0 ? spend / results : 0;
    const prevCpa = prevResults > 0 ? prevSpend / prevResults : 0;
    if (prevCpa > 0 && cpa > 0 && cpa > prevCpa * 1.25) signals.push(`CPA subió ${Math.round((cpa / prevCpa - 1) * 100)}% ($${prevCpa.toFixed(0)} → $${cpa.toFixed(0)})`);
    if (prevResults > 1 && results === 0) signals.push('Sin resultados en este período (antes convertía)');
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="absolute top-2 left-2 z-30 flex flex-col items-start gap-1">
                                  <span className={`text-[9px] font-black px-2 py-1 rounded-lg ${badge.cls} backdrop-blur-sm text-white uppercase tracking-wider`}>{badge.label}</span>
                                  {fatigue?.level === 'high' && (
                                    <span title={fatigue.signals.join(' · ')} className="text-[9px] font-black px-2 py-1 rounded-lg bg-red-600/95 backdrop-blur-sm text-white uppercase tracking-wider flex items-center gap-1 animate-pulse">
                                      <TrendingDown className="w-3 h-3" /> Fatigado — rotar
                                    </span>
                                  )}
                                  {fatigue?.level === 'watch' && (
                                    <span title={fatigue.signals.join(' · ')} className="text-[9px] font-black px-2 py-1 rounded-lg bg-amber-500/95 backdrop-blur-sm text-white uppercase tracking-wider flex items-center gap-1">
                                      <TrendingDown className="w-3 h-3" /> Vigilar
                                    </span>
                                  )}
                                </div>
                              );
                        ...
                        {analyzingTribe ? (
                          ...
                          return (
                            <div className="space-y-5 text-left animate-in fade-in duration-200">
                              {/* Señales de fatiga del anuncio (comparación vs período anterior) */}
                              {(() => {
                                const fatigue = selectedAd ? getAdFatigue(selectedAd.ad) : null;
                                if (!fatigue) return null;
                                return (
                                  <div className={`p-4 rounded-2xl border ${fatigue.level === 'high' ? 'bg-red-50/60 dark:bg-red-950/20 border-red-200/60 dark:border-red-800/30' : 'bg-amber-50/60 dark:bg-amber-950/20 border-amber-200/60 dark:border-amber-800/30'}`}>
                                    <div className="flex items-center gap-2 mb-2">
                                      <TrendingDown className={`w-4 h-4 ${fatigue.level === 'high' ? 'text-red-500' : 'text-amber-500'}`} />
                                      <span className={`text-[12px] font-black uppercase tracking-wider ${fatigue.level === 'high' ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'}`}>
                                        {fatigue.level === 'high' ? 'Fatiga creativa — rotar este anuncio' : 'Señal de fatiga — vigilar'}
                                      </span>
                                    </div>
                                    <ul className="space-y-1">
                                      {fatigue.signals.map((s, i) => (
                                        <li key={i} className="text-[11.5px] font-semibold text-zinc-600 dark:text-zinc-300 flex items-start gap-1.5">
                                          <span className={`w-1 h-1 rounded-full mt-1.5 shrink-0 ${fatigue.level === 'high' ? 'bg-red-400' : 'bg-amber-400'}`} />{s}
                                        </li>
                                      ))}
                                    </ul>
                                  </div>
                              ...
                              {/* Ángulos nuevos sugeridos por IA (solo con fatiga detectada) */}
                              {Array.isArray(metrics.newAngles) && metrics.newAngles.length > 0 && (
                                <div className="p-5 bg-violet-50/60 dark:bg-violet-950/20 border border-violet-200/50 dark:border-violet-800/30 rounded-2xl">
                                  <div className="flex items-center gap-2 mb-3">
                                    <Sparkles className="w-4 h-4 text-violet-500" />
                                    <span className="text-[12px] font-black uppercase tracking-wider text-violet-600 dark:text-violet-400">Ángulos nuevos para reemplazarlo</span>
                                  </div>
                                  <div className="space-y-2.5">
                                    {metrics.newAngles.map((angle: string, i: number) => (
                                      <div key={i} className="flex items-start gap-2.5 p-3 rounded-xl bg-white/70 dark:bg-zinc-900/50 border border-violet-100 dark:border-violet-900/30">
                                        <span className="w-5 h-5 rounded-full bg-violet-500 text-white text-[10px] font-black flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                                        <p className="text-[12px] font-semibold text-zinc-700 dark:text-zinc-200 leading-snug">{angle}</p>
                                      </div>
                                    ))}
                                  </div>
                                </div>
