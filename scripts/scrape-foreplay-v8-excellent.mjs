/**
 * scrape-foreplay-v8-excellent.mjs — Estáticos EXCELENTES por nicho.
 * Igual que v6 pero con piso de calidad: mínimo de likes real (no solo antigüedad)
 * y menos por nicho, para sumar solo lo mejor de lo mejor (no volumen).
 * Filtros: solo imágenes (orFormat[]=image) · corriendo +30 días (runTimeMin=31) ·
 * piso de likes (FP_MIN_LIKES) · por nicho (orNiche[]=X).
 *
 * Uso: node --env-file=.env.local scripts/scrape-foreplay-v8-excellent.mjs
 */
...
const PER_NICHE = Number(process.env.FP_PER || 15);
const POOL_PER_NICHE = Number(process.env.FP_POOL || 100); // candidatos antes de quedarnos con los mejores
const RUNTIME_MIN = Number(process.env.FP_RUNTIME || 31); // días mínimos corriendo
const MIN_LIKES = Number(process.env.FP_MIN_LIKES || 500); // piso de calidad real, no solo antigüedad
const IMAGES_DIR = resolve('./public/scraped_ads/images');
const MANIFEST_PATH = resolve('./public/scraped_ads/manifest.json');
const BATCH = 'foreplay-v8-excellent';
...
async function fetchNiche(niche, token) {
  ...
  for (let pageNum = 0; pageNum < 12 && collected.length < POOL_PER_NICHE; pageNum++) {
    ...
    for (const it of items) {
      ...
      if (likes < MIN_LIKES) continue;                     // piso de calidad real: solo lo probadamente excelente
      seenIds.add(it.id);
...
async function main() {
  console.log('=== Foreplay v8 — estáticos EXCELENTES por nicho ===');
  ...
  for (const niche of NICHES) {
    ...
    for (const ad of top) {
      ...
      const metadata = { scrapedAt: new Date().toISOString(), source: 'foreplay', batch: BATCH, mediaType: 'static_image', foreplayNiches: [niche], daysActive: String(ad._days), likes: Number(ad.likes) || 0, live: !!ad.live, domain: (ad.link_url || '').replace(/^https?:\/\//, '').split('/')[0] };
      const row = {
