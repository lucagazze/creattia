...
WEBSITE VISIBILITY — ${input.includeWebsite && input.displayWebsite ? `The user explicitly requested a visible website, so "${input.displayWebsite}" MUST appear exactly once in this image — this is one of the few things that overrides "nothing that is not in the template". Put it in the template's own URL or footer slot when there is one. WHEN THERE IS NONE, CREATE ONE: set it as a small discreet line centred in the bottom margin, in the ad's own typeface, at the size of the smallest text in the piece. Leaving it out is a failure${input.carousel ? `, and it matters more here than anywhere else: this is one page of a carousel and the others do carry it, so a single page without it reads as a mistake in the set` : ''}.` : 'Do not render any URL, domain, web address, social handle or QR code. Remove any website from the winning template and leave that space clean.'}
