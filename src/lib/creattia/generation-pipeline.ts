...
function quienAparece(mode: PersonMode | undefined, hayFotos: boolean, descripcion?: string) {
	...
	if (mode === 'upload' && hayFotos) {
		return 'The person in it is the one in the supplied photos — that exact face and that exact build, unmistakably them, with their own hair, skin and features. Only the person comes from those photos: what they wear, how they pose, how they are lit and where in the frame they stand all belong to this ad, so dress them for it and put them wherever the composition wants them. Draw them at the highest fidelity you can reach, as a real photograph taken on a professional camera: skin with its own pores, freckles and fine lines, individual strands of hair, catchlights in the eyes, natural subsurface light through the skin, and the shallow depth of field of a real lens. Do this even if the supplied photos come soft, small, compressed or badly lit — you are re-photographing that person, not upscaling their snapshot. Never smooth the skin, never even out the tone, never let it drift into the plastic, waxy look of a render.';
	}
