...
export type LayoutAnalysis = {
	...
	/** Si es un rótulo con línea guía, la parte del producto que señala. */
		 ...
		 */
		/** El color de estas letras, en hexadecimal leído de la imagen. */
		textColor?: string;
		/** El relleno de su cápsula o tarjeta, si tiene una propia. */
		boxColor?: string;
		setIn?: string;
		messageRole?: string;
		replacement?: string;
...
export async function analyzeReferenceLayout(keys: { openAIKey?: string; googleKey?: string }, input: {
	...
	const languageRule = input.language && LANGUAGE_NAMES[input.language]
		? `TARGET LANGUAGE IS ${LANGUAGE_NAMES[input.language].toUpperCase()} AND IT IS NOT NEGOTIABLE. Every replacement string you produce — headline, description, CTA, every text zone — must be written in ${LANGUAGE_NAMES[input.language]}, even when the template you are analysing is written in a different language. Do NOT copy or keep the template's original language. Translate and adapt the message instead. Set "language" to "${input.language}". This applies ONLY to what a reader will see in the finished ad. The measurements and descriptions you write for the renderer — "where", "setIn", "compositionGeometry", "styleNotes", "typography", "renderingMedium", "productPlacement" — stay in ENGLISH always: they are handed to an English instruction set, and a measurement in another language reads as a weaker hint there.${grammarRule}`
...
- BEFORE/AFTER (CRITICAL): when the two sides are a before and an after, the question that decides the whole ad is WHERE. Both sides must show the same place where the TARGET product does its work — not the body part the template happened to use. A toenail treatment shows a toe; a boxer brief does its work on the groin and inner thigh, so that is what its before and after show, with the garment worn on the after side. Answering this with a generic body part, or with a close-up of skin that has nothing to do with the product, produces an ad about nothing. Write that place into "replaceWith" for BOTH halves, explicitly.
	...
	const validate = (raw: string | null | undefined): LayoutAnalysis | null => {
		try {
			...
			parsed.textZones = parsed.textZones.map((zone: any) => ({
				...
				// Solo se acepta un hexadecimal completo: un "blanco roto" colado acá
				// volvería a ser una palabra, que es justo lo que se quiso sacar.
				textColor: /^#[0-9a-f]{6}$/i.test(String(zone?.textColor || '').trim()) ? String(zone.textColor).trim().toUpperCase() : undefined,
				boxColor: /^#[0-9a-f]{6}$/i.test(String(zone?.boxColor || '').trim()) ? String(zone.boxColor).trim().toUpperCase() : undefined,
...
WHAT THE TWO HALVES SHOW — Both halves show THE SAME PLACE: the exact part of the body, the surface or the situation where ${productLabel} is actually used, from the same angle and the same distance, so the ONLY difference between them is the state. The template's own body part is not that place unless this product is used there — a toenail treatment's toe becomes wherever THIS product does its work, and picking a generic body part instead is how the ad stops being about anything. If the product is worn or applied, it is VISIBLE on the AFTER side: that is what the reader is being shown to buy.`
	...
	if (zones.length) {
		textSwap = zones.map((zone, index) => {
			...
			// El color medido va aparte y en hexadecimal: dentro de la prosa de la
			// medida se re-interpreta y el tono se corre.
			const tono = (zone as any).textColor
				? `; letters exactly ${(zone as any).textColor}${(zone as any).boxColor ? ` on its own ${(zone as any).boxColor} shape` : ''}`
				: '';
			...
			const encaje = largo || lineas || compuesto
				? ` — the original is ${largo} characters${lineas}${compuesto || '; the replacement must occupy the same block at the same size'}${tono}${caja}`
				: caja;
...
The new ad must be shot the same way. A flat, evenly lit product on a plain background is a failure if the template used directional light and shallow depth.`
   ...
   If an area calls for a shot the supplied photos do not contain, use the closest one cropped to that frame rather than inventing the missing view; if none fits, close the area with the template's own background treatment.`;
	...
	const creativeDecisionBlock = creativeDecisions.length
		? `\nCONTEXTUAL CREATIVE DECISIONS (CRITICAL) — The visual analysis found details that change how believable the ad is. Where the advertiser answered, their answer IS the decision and there is nothing left to weigh. Where they did not, use the default and do not invent an unrelated subject, brand or claim:\n${creativeDecisions.map((decision, index) => { const respuesta = decision.directive?.trim(); return `   - Decision ${index + 1} [${decision.type || 'other'}]${decision.title ? ` ${decision.title}` : ''}${decision.where ? ` (${decision.where})` : ''}: detected — ${decision.description || decision.question || 'contextual visual element'}. ${respuesta ? `THE ADVERTISER DECIDED: ${respuesta}. Do exactly that.` : `Nobody answered, so: ${decision.defaultStrategy || 'make the most natural, same-category and production-ready choice.'}`}`; }).join('\n')}`
		: '';
	/**
...
MARGINS ARE PART OF THE DESIGN — Keep the same clear space between every text block and the edges as the template has. No letter may touch or cross an edge, and no word may be clipped or cut to make a line fit: "CON CAMELOT" never becomes "CON CAM". Remove a whole word and rewrite the line instead. Same inside a card, pill, badge or button: the text keeps its padding.`;
	...
	const layoutFidelityRule = `\nLAYOUT FIDELITY OF TEXT BLOCKS (CRITICAL) — Every text block keeps the place it has in the template: same corner, same side, same distance from the edges, same width, the same horizontal anchor — a block centred on the canvas axis stays centred there, and one flush to an edge stays flush to that edge, even when the new text is shorter or longer than the original — and the same relationship with the photo — a block overlapping the image on the left overlaps on the left. Mirroring or moving one breaks the composition that made the original work. Reproduce too any highlight, marker, underline or boxed word applied to part of a text: those marks tell the reader where to look.`;
   ...
   KEEP THE GRID UNIFORM — This rule overrides anything said earlier about re-shooting the image areas. If the template's cells are packshots on a plain background, EVERY cell stays a packshot on a plain background: same framing, same distance, same shadow, same margin. No cell may show hands, a person, a tool, a workbench, a step of the process or a lifestyle scene — a catalogue grid shows the PRODUCTS FOR SALE and nothing else — and the background may not vary between cells. The grid reads as a catalogue because it is regular; one different cell breaks it.` : ''}`
	...
	const qualityRule = `\nOUTPUT QUALITY (CRITICAL) — Do not downscale, blur or soften the final image. Keep edges, textures and shadows crisp and cleanly rendered in the template's own medium. The result must look production-ready and stay crisp at 100% zoom.`;
...
NOTHING THAT IS NOT IN THE TEMPLATE (READ THIS FIRST) — You may only reproduce elements that are VISIBLE in the winning ad. Before adding anything, look for it in the template; if it is not there, it does not go in. This applies especially to what gets added by reflex: a web address, domain, handle or QR; a logo, wordmark or brand line; a badge, seal, ribbon, laurel, star rating or certification; a guarantee, discount or shipping promise; an extra caption or icon. The advertiser's website and brand name are given to you as CONTEXT so you can write honest copy — they are not instructions to print them. An ad that shows one element more than the template is wrong even if that element looks good. Equally, do not leave large empty areas the template does not have: every band of the canvas carries what the template's equivalent band carries.
