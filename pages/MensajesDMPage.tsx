...
import {
  ...
  Search, MessageSquare, Clock, CheckCheck, Check, Filter
} from 'lucide-react';
...
export default function MensajesDMPage() {
        ...
        if (igDMs?.data) {
          igDMs.data.forEach((conv: any) => {
            ...
            items.push({
              id: conv.id,
              platform: 'instagram',
              username: other?.name || lastMsg?.from?.username || 'Usuario de Instagram',
        ...
        if (fbDMs?.data) {
          fbDMs.data.forEach((conv: any) => {
            ...
            items.push({
              id: conv.id,
              platform: 'facebook',
              username: other?.name || lastMsg?.from?.name || 'Usuario de Messenger',
              lastMessage: lastMsg?.message || '(Archivo adjunto)',
              timestamp: lastMsg?.created_time || conv.updated_time,
              isPending: !isFromMe,
              unread: conv.unread_count > 0,
  ...
  const loadHistory = useCallback(async (conv: ConvItem) => {
    ...
    try {
      ...
      const fallback = conv.rawItem?.messages?.data
  ...
  const generateDraft = async () => {
    ...
    setReplyError(null);
    try {
      const last15 = convMessages.slice(-15).map(m => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
        ...
        <p className="text-[13px] text-zinc-500 max-w-sm">
          Conectá tu cuenta de Instagram o Facebook en la configuración.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-4 border-b border-zinc-200/60 dark:border-zinc-800/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            ...
            {showPendingOnly
              ? `Mostrando ${filtered.length} pendiente${filtered.length !== 1 ? 's' : ''}`
      ...
      {loading ? (
        <div className="pt-6 flex-1">
          <EmailLoader
            loading={loading}
            color="#8b5cf6"
            labels={['Cargando DMs de Instagram...', 'Cargando Messenger de Facebook...']}
          />
        </div>
      ) : (
            ...
            {(igError || fbError) && (
              <div className="mx-3 mt-2 p-2.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 rounded-xl flex items-start gap-2">
                ...
                <p className="text-[10px] text-amber-700 dark:text-amber-400 font-semibold leading-snug">
                  {[igError && `Instagram: ${igError}`, fbError && `Facebook: ${fbError}`].filter(Boolean).join(' | ')}
                </p>
              </div>
            )}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1">
                          <span className={`text-[12.5px] font-black truncate ${
                            conv.isPending ? 'text-zinc-900 dark:text-white' : 'text-zinc-500 dark:text-zinc-400'
                          }`}>
                            {conv.username}
                          </span>
                        ...
                        <div className="flex items-center justify-between gap-1 mt-0.5">
                          <p className={`text-[11px] truncate ${
                            conv.isPending
                              ...
                              : 'text-zinc-400 font-medium italic'
                          }`}>
                            {conv.lastMessage}
                          </p>
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-zinc-50/30 dark:bg-zinc-950/10">
            {!selectedConv ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center p-8">
                ...
                <div>
                  <h3 className="text-[15px] font-black text-zinc-600 dark:text-zinc-400">
                    Seleccioná una conversación
                  </h3>
                  <p className="text-[12px] text-zinc-400 mt-1">
                    Hacé click en un chat de la izquierda para ver el historial y responder.
                  </p>
                  {pendingCount > 0 && (
                    <button
              ...
              <>
                ...
                <div className="px-5 py-3 border-b border-zinc-150 dark:border-zinc-800 flex items-center gap-3 bg-white dark:bg-zinc-900 flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[14px] font-black text-zinc-900 dark:text-white truncate">
                  ...
                  {loadingMessages ? (
                    ...
                    convMessages.map((msg: any) => {
                      ...
                      const timeStr = msg.created_time
                        ? new Date(msg.created_time).toLocaleString('es-AR', {
