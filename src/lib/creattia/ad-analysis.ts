...
The new ad must be shot the same way. A flat, evenly lit product on a plain background is a failure if the template used directional light and shallow depth.`
   ...
   KEEP THE GRID UNIFORM — If the template's cells are packshots on a plain background, EVERY cell stays a packshot on a plain background: same framing, same distance, same shadow, same margin. Do not turn one cell into a lifestyle, workshop or in-use photo, and do not vary the background between cells. The grid reads as a catalogue because it is regular; one different cell breaks it.` : ''}`
		: '';
