...
import {
  TrendingUp, MessageCircle, Inbox, Send, Clock, CheckCircle, Users, ChevronDown, Calendar, Loader2, AlertCircle, Activity, User, HelpCircle, Instagram, Heart, ArrowUpRight
} from 'lucide-react';
import { chatwoot } from '../services/chatwoot';
import { metaAds, getPrevPeriod, today, daysAgo, presetToRange } from '../services/metaAds';
...
export default function InformesPage() {
  ...
  // Fetch organic social data + save daily snapshot
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        // Fetch snapshots history from Supabase
        ...
        // Fetch current profiles from Meta API
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-fade-in pb-20 pt-6 px-4 md:px-6 lg:px-8">
      ...
      <div className="mb-10">
        ...
        {loadingSocial && !igProfile && !fbProfile ? (
          ...
          <>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={igSnaps} margin={{ top: 2, right: 2, bottom: 2, left: 2 }}>
                          ...
                          <Tooltip
                            ...
                            formatter={(v: any) => [fmtN(v), 'Seguidores']}
                            labelFormatter={(l) => l}
                          />
                        </LineChart>
                      </ResponsiveContainer>
                    ...
                    <div className="mt-4 h-16">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={fbSnaps} margin={{ top: 2, right: 2, bottom: 2, left: 2 }}>
                          ...
                          <Tooltip
                            contentStyle={{ fontSize: 11, borderRadius: 8, border: 'none', background: '#18181b', color: '#fff' }}
                            formatter={(v: any) => [fmtN(v), 'Me gusta']}
                            labelFormatter={(l) => l}
                          />
                        </LineChart>
                      </ResponsiveContainer>
            ...
            {!igProfile && !fbProfile && !loadingSocial && (
              <div className="bg-zinc-50 dark:bg-white/[0.02] border border-black/[0.03] dark:border-white/[0.03] rounded-2xl p-8 text-center">
        ...
        <div className="border-b border-zinc-100 dark:border-white/[0.05] my-10" />
      </div>
