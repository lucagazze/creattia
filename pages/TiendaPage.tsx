          ...
          return (
            <div key={d} className="relative py-0.5 flex items-center justify-center w-full">
              ...
              {hasRange && (isInRange || (isHovering && d !== displayStart && d !== displayEnd)) && (
                <div className="absolute inset-y-0.5 left-0 right-0 bg-pink-50 dark:bg-pink-500/10" />
              )}
              {hasRange && isStart && (
                <div className="absolute inset-y-0.5 right-0 left-1/2 bg-pink-50 dark:bg-pink-500/10" />
              )}
              {hasRange && isEnd && (
                <div className="absolute inset-y-0.5 left-0 right-1/2 bg-pink-50 dark:bg-pink-500/10" />
              )}
              ...
              <button
                ...
                } ${isToday && !isSelected && !(since && !until && hovering && (d === since || d === hovering)) ? 'text-pink-600 dark:text-pink-500 ring-1 ring-pink-100 dark:ring-pink-900/30' : ''}`}
              >
                {d.split('-')[2]}
              </button>
            </div>
    ...
    <div className="w-full animate-fade-in pb-20 pt-4 md:pt-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 print:hidden">
        ...
        <div className="flex items-center gap-3 print:hidden">
          <div className="flex items-center bg-white dark:bg-zinc-900 border border-black/[0.06] dark:border-white/[0.06] rounded-full px-1 py-0.5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_1px_4px_rgba(0,0,0,0.06)] h-9 relative self-start w-fit" ref={datePickerRef}>
              <button 
                  ...
                  <div className="w-full md:w-[150px] border-b md:border-b-0 md:border-r border-zinc-50 dark:border-zinc-800 p-1.5 md:p-3 grid grid-cols-3 md:flex md:flex-col gap-1">
                    {[
                      ...
                      <button
                        ...
                        className={`flex-shrink-0 text-center md:text-left px-1 py-1 md:px-2.5 md:py-1 rounded-[4px] md:rounded-[10px] text-[8.5px] sm:text-[9px] md:text-[10px] font-bold transition-all whitespace-nowrap ${pendingPreset === p.id ? 'bg-pink-600 text-white shadow-md shadow-pink-200/40 dark:shadow-none' : 'text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}
                      >
                        {p.label}
                      </button>
                    ))}
